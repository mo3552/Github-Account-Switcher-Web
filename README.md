...
-   **중복 실행 방지**: `app.requestSingleInstanceLock()`을 사용하여 동시 실행 차단
-   **기존 인스턴스 포커스**: 두 번째 인스턴스 실행 시도 시 기존 윈도우 포커스
-   **최소화된 윈도우 복원**: 최소화된 상태의 기존 인스턴스 자동 복원
-   **데이터 일관성 보장**: 여러 인스턴스 간 데이터 충돌 방지
